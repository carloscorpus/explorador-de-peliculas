...
export const DisplayMovieDetail = () => {
	...
	const { data } = useMovieDetail(movie);
	const navegite = useNavigate();

	return (
		<ContainerApp disableGutters={false} component={'main'}>
			<CardMain
				sx={{ p: '1em', display: 'flex', alignItems: 'start', flexDirection: { xs: 'column', md: 'row' } }}
			>
				<CardMediaMain
				...
				<Box>
					<CardContentMain>
						...
						<TitleMain component={'p'}>{data?.Plot !== 'N/A' ? data?.Plot : 'No description'}</TitleMain>
						<CardListMain>
							<ListItemCard type="Premier" data={data?.Released} iconCard={<CalendarMonthIcon />} />
							...
							<ListItemCard type="Language" data={data?.Language} iconCard={<LanguageOutlinedIcon />} />
							<ListItemCard type="Awards" data={data?.Awards} iconCard={<EmojiEventsOutlinedIcon />} />
							<ListItemCard
								type="BoxOffice"
								data={data?.BoxOffice}
